...
namespace kfutils.noise {
    ...
    public class HeightNoiseMap {
        ...
        protected const float SQRT_THREE = 1.73205080757f;
        protected const float SCALE_FOR_ZERO_TO_ONE = SQRT_THREE * 0.5f;
        int xOff, zOff, sizex, sizez, interval, cutoff, currentInterval, layer;
        float[,] field;
        float divisor;
        protected float scale = SCALE_FOR_ZERO_TO_ONE;
        ...
        protected void ProcessOne(SpatialHash rand) {
        ...
        protected float CalcLoc(Vec2D from, Vec2D at, float ci) {
            double dx = at.x / ci;
            double dy = at.y / ci;
        ...
        protected float Fade(float val) {
            return val * val * val * (val * (val * 6 - 15) + 10);
        }


        protected float FullFade(float val) {
