    ...
    public class ItemDropUI : InventorySlotUI {
        ...
        public override bool SwapWith(InventorySlotUI other) {
            if((other.item != null) && (other.item.item != null) && (other.item.item.InWorld != null)) {
                ...
                if(shouldThrow) {
                    other.item.DropItemInWorld(pc.PlayerCam.transform, 0.5f, 5.0f * (1  + pc.attributes.jumpForce) * Mathf.Sqrt(other.item.item.Weight));
                } else {
                    other.item.DropItemInWorld(pc.PlayerCam.transform, 0.5f);
                }
